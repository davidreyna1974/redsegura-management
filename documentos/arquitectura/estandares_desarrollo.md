...
| L07 | **openapi-generator (servicios Java, ADR-05):** (a) declarar cada path-param **por operación**, no a nivel-path (el generador lo descarta si la operación tiene sus propios `parameters`); (b) si la ruta del proyecto tiene espacios, copiar el spec a una ruta temporal sin espacios antes de generar; (c) la validación (p. ej. `pattern` de IP) va **en el contrato**; (d) excluir el código generado de JaCoCo/Checkstyle. Config de referencia: `asset-inventory-service/pom.xml` | Cableado de openapi-generator en `asset-inventory` | global (servicios Java) |
