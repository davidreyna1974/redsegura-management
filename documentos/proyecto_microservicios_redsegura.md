...
- **RNF-32:** **Integración cross-service (golden path) obligatoria.** Cada **vector de interacción** entre un microservicio y otro(s) —evento producido→consumido, o llamada API/otro mecanismo— debe **validarse en vivo, de extremo a extremo**, sobre el artefacto desplegado (Docker Compose con **ambos** servicios reales, broker/BD/IdP reales), además del contract testing (RNF-21, que valida el contrato en aislamiento). Si un microservicio interactúa con **varios**, se prueba **cada vector** por separado. Cada vector se registra en la **matriz de interacción** (`qa/matriz_interaccion.md`) y se documenta con su **golden path** (diseño + ejecución + resultados). Un vector cuyo contraparte aún no existe se registra como **diferido con disparador** (se valida al construirse la contraparte). El cumplimiento lo **obliga un gatekeeper** (`scripts/check_golden_paths.py`) en el gate de release: ningún vector con ambas partes existentes puede quedar sin golden path ejecutado.
